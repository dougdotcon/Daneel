...
"""DeepSeek model implementation for Parlant."""
...
class DeepSeekSchematicGenerator(SchematicGenerator[T]):
    """Schematic generator for DeepSeek models."""
    ...
    def __init__(
        ...
        self.model = model
        self._logger = logger
        self._tokenizer = DeepSeekEstimatingTokenizer()
    ...
    async def generate(
        ...
        """
        with self._logger.scope("DeepSeekSchematicGenerator"):
    ...
    async def _do_generate(
        ...
        if self._is_ollama:
            ...
            return await self._generate_with_deepseek_api(prompt, filtered_hints)
    ...
    async def _generate_with_deepseek_api(
        self,
        prompt: str,
        hints: Mapping[str, Any] = {},
    ) -> SchematicGenerationResult[T]:
        ...
        Returns:
            The generated response
        """
        # This is a placeholder for the DeepSeek API implementation
        # In a real implementation, you would use the DeepSeek API client
        
        self._logger.error("DeepSeek API implementation not available")
        raise NotImplementedError("DeepSeek API implementation not available")


class DeepSeekEmbedder(Embedder):
    """Embedder for DeepSeek models."""
    ...
    async def embed(
        ...
        if self._is_ollama:
            ...
            return await self._embed_with_deepseek_api(texts, hints)
    ...
    async def _embed_with_deepseek_api(
        self,
        texts: list[str],
        hints: Mapping[str, Any] = {},
    ) -> EmbeddingResult:
...
class DeepSeekModerationService(ModerationService):
    """Moderation service for DeepSeek models."""
...
class DeepSeekService(NLPService):
    """NLP service for DeepSeek models."""
    ...
    def __init__(
        ...
        # If model_id is not provided, use the first available DeepSeek model
        if not model_id:
            deepseek_models = model_manager.list_models(ModelType.DEEPSEEK)
            if not deepseek_models:
                raise ValueError("No DeepSeek models available")
        ...
        self._logger.info(f"Initialized DeepSeekService with model {self.model.id}")
    ...
    async def get_schematic_generator(self, t: type[T]) -> DeepSeekSchematicGenerator[T]:
        ...
        Returns:
            Schematic generator
        """
        return DeepSeekSchematicGenerator[t](self.model, self._logger)  # type: ignore
    ...
    async def get_embedder(self) -> Embedder:
        ...
        Returns:
            Embedder
        """
        return DeepSeekEmbedder(self.model, self._logger)
    ...
    async def get_moderation_service(self) -> ModerationService:
        ...
        Returns:
            Moderation service
        """
        return DeepSeekModerationService(self.model, self._logger)
